...
export default function TaskDetailsPage() {
  ...
  const canEdit = session?.user?.id === task?.assignments[0]?.assignee.id
      ...
      <div className="container mx-auto px-4 max-w-4xl">
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <div className="flex items-center mt-1">
                  <Avatar className="h-8 w-8 mr-2">
                    <AvatarImage src={task.assignments[0]?.assignee.image} alt={task.assignments[0]?.assignee.name} />
                    <AvatarFallback>{task.assignments[0]?.assignee.name[0]}</AvatarFallback>
                  </Avatar>
                  <span>{task.assignments[0]?.assignee.name}</span>
                </div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <p className="mt-1">{task.startDate ? new Date(task.startDate).toLocaleDateString() : 'Not set'}</p>
              </div>
              <div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {task.subtasks.map((subtask) => (
              <Card key={subtask.id} className="mb-4">
                <CardContent className="p-4">
                  {editingSubtask?.id === subtask.id ? (
                    <>
                      <Input
